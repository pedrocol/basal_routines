...
subroutine ocean_velocity_advect_init(Grid, Domain, Time, obc, hor_grid, debug)
  ...
  id_basal_u       = register_diag_field ('ocean_model', 'basal_u', Grd%vel_axes_uv(1:2), &
                     ...
                     missing_value=missing_value, range=(/-1e10,1e10/))
  id_icb_u       = register_diag_field ('ocean_model', 'icb_u', Grd%vel_axes_uv(1:2), &
                     Time%model_time, 'icb on u-cell', '(kg/m^3)*(m/s)',                &
                     missing_value=missing_value, range=(/-1e10,1e10/))
  !Pedro
...
subroutine vert_advection_of_velocity(Time, Adv_vel, Velocity, pme, river, &
                                      ...
                                      basal3d, ubasal3d, energy_analysis_step)
...
!  real, dimension(isd:,jsd:,:),   intent(in)    :: icb3d
  ...
  real, dimension(isd:ied,jsd:jed)   :: icb_u !Pedro
  real, dimension(isd:ied,jsd:jed,nk)   :: icb3d_u !Pedro
  ...
  if(advection_scheme==VEL_ADVECT_2ND_ORDER) then 
    call vert_advection_centered(Time, Adv_vel, Velocity, pme, pme_u, river, river_u, &
                                 ...
                                 basal3d, ubasal3d, basal3d_u, energy_analysis_step) !Pedro
  elseif(advection_scheme==VEL_ADVECT_UPWIND) then
    call vert_advection_upwind(Time, Adv_vel, Velocity, pme_u, river_u, basal_u, icb_u,   &
         ...
         do j=jsc,jec
            do i=isc,iec
               surf_accel(i,j,n) = Grd%umask(i,j,1)*(pme_u(i,j)*upme(i,j,n) + river_u(i,j)*uriver(i,j,n) & 
                                                     + basal3d_u(i,j,1)*ubasal3d(i,j,n,1) &
                                                     + icb3d_u(i,j,1)*uicb3d(i,j,n,1)) !Pedro
  ...
  call diagnose_2d_u(Time, Grd, id_icb_u, icb_u(:,:)) !Pedro
...
subroutine vert_advection_centered(Time, Adv_vel, Velocity, pme, pme_u, river, river_u, &
                                   basal, basal_u, icb, icb_u, upme, uriver, ubasal, uicb, &
                                   basal3d, ubasal3d, basal3d_u, energy_analysis_step)
  ...
  real, dimension(isd:,jsd:),   intent(in)    :: icb_u
  !Pedro
  real, dimension(isd:,jsd:,:), intent(in)    :: upme
  real, dimension(isd:,jsd:,:), intent(in)    :: uriver 
  real, dimension(isd:,jsd:,:), intent(in)    :: ubasal !Pedro
  ...
  real, dimension(isd:,jsd:,:,:), intent(in)       :: ubasal3d
  real, dimension(isd:,jsd:,:),   intent(in)    :: basal3d
  real, dimension(isd:,jsd:,:),   intent(in)       :: basal3d_u
  ...
  if(horz_grid == MOM_BGRID) then 
     ...
     do n=1,2
        ...
        do j=jsc,jec
           do i=isc,iec
              ...
              ft1(i,j) = -pme_u(i,j)*upme(i,j,n) -river_u(i,j)*uriver(i,j,n) - basal3d_u(i,j,1)*ubasal3d(i,j,1,n) &
                         - icb_u(i,j)*uicb(i,j,n)
              !Pedro
              ft2(i,j) = 0.0
           enddo
        enddo
     ...
     do j=jsc,jec
        do i=isc,iec
           ...
           ft1(i,j) = -onefourth*Grd%tmasken(i,j,1,n)*(pme(i,j)  +pme(i+1,j))  *(upme(i,j,n)  +upme(i,j-1,n)  ) &
                      -onefourth*Grd%tmasken(i,j,1,n)*(river(i,j)+river(i+1,j))*(uriver(i,j,n)+uriver(i,j-1,n)) &
                      -onefourth*Grd%tmasken(i,j,1,n)*(basal3d(i,j,1)+basal3d(i+1,j,1))*(ubasal3d(i,j,1,n)+ubasal3d(i,j-1,1,n)) &
                      -onefourth*Grd%tmasken(i,j,1,n)*(icb(i,j)+icb(i+1,j))*(uicb(i,j,n)+uicb(i,j-1,n))
        ...
        do j=jsc,jec
           do i=isc,iec
              ft2(i,j) = onefourth*(Adv_vel%wrho_bt(i,j,k)+Adv_vel%wrho_bt(i+1,j,k)) &
                         ...
                         +onefourth*Grd%tmasken(i,j,k,n)*(basal3d(i,j,k)+basal3d(i+1,j,k))&
                                  *(ubasal3d(i,j,k,n)+ubasal3d(i,j-1,k,n))
                         !+onefourth*Grd%tmasken(i,j,k,n)*(icb3d(i,j,k)+icb3d(i+1,j,k))&
                         !         *(uicb3d(i,j,k,n)+uicb3d(i,j-1,k,n))
     ...
     do j=jsc,jec
        do i=isc,iec
           ...
           ft1(i,j) = -onefourth*Grd%tmasken(i,j,1,n)*(pme(i,j)  +pme(i,j+1))  *(upme(i,j,n)  +upme(i-1,j,n)  ) &
                      ...
                      -onefourth*Grd%tmasken(i,j,1,n)*(icb(i,j)+icb(i,j+1))*(uicb(i,j,n)+uicb(i-1,j,n))
           !Pedro
           ft2(i,j) = 0.0
        enddo
     enddo
        ...
        do j=jsc,jec
           do i=isc,iec
              ft2(i,j) = onefourth*(Adv_vel%wrho_bt(i,j,k)+Adv_vel%wrho_bt(i,j+1,k)) &
                                  *(Velocity%u(i,j,k,n,tau) + kmask(k)*Velocity%u(i,j,kp1,n,tau)) &
                         + onefourth*Grd%tmasken(i,j,k,n)*(basal3d(i,j,k)+basal3d(i,j+1,k)) &
                                  *(ubasal3d(i,j,k,n)+ubasal3d(i-1,j,k,n))
                         !+ onefourth*Grd%tmasken(i,j,k,n)*(icb3d(i,j,k)+icb3d(i,j+1,k)) &
                         !         *(uicb3d(i,j,k,n)+uicb3d(i-1,j,k,n))
              wrk1_v(i,j,k,n) = Grd%tmasken(i,j,k,n)*(ft1(i,j)-ft2(i,j))
              ft1(i,j) = ft2(i,j)
           enddo
        enddo
...
! </DESCRIPTION>
! 
subroutine vert_advection_upwind(Time, Adv_vel, Velocity, pme_u, river_u, basal_u, icb_u, &
                                 upme, uriver, ubasal, uicb,&
                                 basal3d, ubasal3d, basal3d_u, energy_analysis_step)
  ...
  real, dimension(isd:,jsd:),   intent(in)    :: icb_u !Pedro
  real, dimension(isd:,jsd:,:), intent(in)    :: upme
  real, dimension(isd:,jsd:,:), intent(in)    :: uriver 
  real, dimension(isd:,jsd:,:), intent(in)    :: ubasal !Pedro
  real, dimension(isd:,jsd:,:), intent(in)    :: uicb !Pedro
  ...
  real, dimension(isd:,jsd:,:,:), intent(in)       :: ubasal3d
  real, dimension(isd:,jsd:,:),   intent(in)    :: basal3d
  real, dimension(isd:,jsd:,:),   intent(in)       :: basal3d_u !Pedro
  ...
  do n=1,2
     ...
     do j=jsc,jec
        do i=isc,iec
           ...
           !ft1(i,j) = -pme_u(i,j)*upme(i,j,n) -river_u(i,j)*uriver(i,j,n) -basal_u(i,j)*ubasal(i,j,n)
           ft1(i,j) = -pme_u(i,j)*upme(i,j,n) - river_u(i,j)*uriver(i,j,n) &
                      - basal3d_u(i,j,1)*ubasal3d(i,j,1,n) - icb_u(i,j)*uicb(i,j,n)
           !Pedro
           ft2(i,j) = 0.0
        enddo
     enddo
           ...
           do i=isc,iec
              ...
              ft2(i,j) = (wneg*Velocity%u(i,j,k,n,taum1) + kmask(k)*wpos*Velocity%u(i,j,kp1,n,taum1)) &
                         *Grd%umask(i,j,k)*Grd%umask(i,j,kp1) &
                         + basal3d_u(i,j,k)*ubasal3d(i,j,k,n)! + icb3d_u(i,j,k)*uicb3d(i,j,k,n)
              wrk1_v(i,j,k,n) = Grd%umask(i,j,k)*(ft1(i,j)-ft2(i,j))
              ft1(i,j) = ft2(i,j)
           enddo
        enddo
