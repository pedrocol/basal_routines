...
subroutine get_ocean_sbc(Time, Ice_ocean_boundary, Thickness, Dens, Ext_mode, T_prog, Velocity, &
                         pme, melt, river, runoff, basal, icb, calving, upme, uriver, ubasal, &
                         uicb, swflx, swflx_vis, patm, ubasal3d, uicb3d)
...
!Pedro
  ...
  real, dimension(isd:,jsd:,:),   intent(inout) :: uicb
  real, dimension(isd:,jsd:,:,:),   intent(inout) :: uicb3d
!Pedro
...
!            Velocity%stokes_depth(ii,jj)     = Ice_ocean_boundary%stokes_depth(i,j)*Grd%umask(ii,jj,1)
  ...
  do n = 1, size(upme,3)
     do j = jsc, jec
        do i = isc, iec
           ...
           uicb(i,j,n) = Velocity%u(i,j,1,n,tau)   ! velocity of icb water 
           !Pedro
        enddo
     enddo
  enddo
     ...
     do j = jsc, jec
        do i = isc, iec
           do k=1,nk
              ...
              uicb3d(i,j,k,n) = Velocity%u(i,j,k,n,tau)   ! velocity of icb water
           enddo
        enddo
     enddo
  enddo
  ...
  if (use_waterflux) then
      ...
      if(use_basal_module) then
        ...
        if (calv_id < 1) then
          call mpp_error(FATAL,'==>Error: in ocean_sbc:  basal file')
        endif
        wrk1  = 0.0
       ...
       !Import iceberg field
        if (calv2_id < 1) then
          call mpp_error(FATAL,'==>Error: in ocean_sbc:  icb file')
        endif
        wrk1  = 0.0
       ! get basal value for current time
      ...
      if(land_model_heat_fluxes) then  
          ...
          do j=jsc,jec
             do i=isc,iec
                ...
                T_prog(index_temp)%ticb(i,j) = 0.0
                ...
                !Pedro !We first set default values
                ...
                if(icb(i,j) > epsln) then !Pedro
                    T_prog(index_temp)%ticb(i,j) = &
                    min(100.0, max(-273.15,T_prog(index_temp)%icb_tracer_flux(i,j)/icb(i,j)))
                endif
                !Pedro
             enddo
          enddo
          ...
          do j=jsc,jec
             ...
             !Pedro
                ...
                T_prog(index_temp)%icb_tracer_flux(i,j)= &
                   Grd%tmask(i,j,1)*T_prog(index_temp)%ticb(i,j)*icb(i,j)
             !Pedro
             enddo
          enddo
          ...
          if(index_redist_heat > 0) then
            do j=jsc,jec
               ...
               !Pedro
                 ...
                 T_prog(index_redist_heat)%icb_tracer_flux(i,j) = &
                   Grd%tmask(i,j,1)*T_prog(index_redist_heat)%ticb(i,j)*icb(i,j)
               !Pedro
               enddo
            enddo
          endif           
          ...
          if(index_added_heat > 0) then
            do j=jsc,jec
               do i=isc,iec
                 ...
                 T_prog(index_added_heat)%icb_tracer_flux(i,j) = &
                   Grd%tmask(i,j,1)*T_prog(index_added_heat)%ticb(i,j)*icb(i,j)
                 !Pedro
               enddo
            enddo
          endif           
  ...
  call ocean_tpm_sbc(Dom, Grd, T_prog(:), Time, Ice_ocean_boundary%fluxes, runoff, &
                     basal, icb, isc_bnd, iec_bnd, jsc_bnd, jec_bnd)
