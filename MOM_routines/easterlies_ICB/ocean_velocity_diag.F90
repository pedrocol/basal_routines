...
subroutine energy_analysis (Time, Thickness, Ext_mode, Adv_vel, Dens,    & 
                            ...
                            uicb, visc_cbu, visc_cbt, visc_cbu_form_drag, Velocity, &
  ...
  if(horz_grid == MOM_BGRID) then 
     ...
     icb_u(:,:) = REMAP_BT_TO_BU(icb) !Pedro
     basal3d_u  = 0.0
     icb3d_u  = 0.0
     do j=jsc,jec
        do i=isc,iec
  ...
  do n=1,2
     do j=jsc,jec
        do i=isc,iec
           ...
           term       = pme_u(i,j)*upme(i,j,n) + river_u(i,j)*uriver(i,j,n) &
                      + basal3d_u(i,j,1)*ubasal3d(i,j,1,n) + icb3d_u(i,j,1)*uicb3d(i,j,1,n) !Pedro
           term       = term*boxarea 
