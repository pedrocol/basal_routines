...
subroutine ocean_tracer_diagnostics(Time, Thickness, T_prog, T_diag, Dens, &
  ...
  if (nint(dtts) /= 0) then 
    call mass_conservation   (Time, Thickness, Ext_mode, pme, runoff, calving, basal, icb) !Pedro
...
subroutine tracer_change (Time, Thickness, T_prog, T_diag, Ext_mode, &
  ...
  icb_input      = 0.0 ! total mass of icb (runoff) water input to ocean over time step (kg)
  !Pedro
  ...
  ! mass of icb (runoff) water input to the ocean
  ...
  mass_error = mass_change-pme_input-melt_input-runoff_input-calving_input &
               - basal_input - icb_input - obc_input-mass_source
  !Pedro
  ...
  do n=1,num_prog_tracers
     ...
     tracer_input_icb  = 0.0 ! tracer quantity input to ocean via icb (runoff)
     ...
     !Pedro
     !Perhaps the basalf fw can be negative at some point, we make here the same
     !consideration as for the runoff
     tmp(:,:) = 0.0
     do j=jsc,jec
        do i=isc,iec
           if(basal(i,j) > 0) then
     ...
     tracer_input_total =   tracer_input_stf    + tracer_input_btf     + tracer_input_otf &
                          ...
                          + tracer_input_basal + tracer_input_icb  &
...
!     if (use_blobs) tracer_error = tracer_error + dmasstracer_L2
     ...
     if (T_prog(n)%name =='temp') then
         ...
         write (stdoutunit,'(a,es24.17,a)') ' Heat input via icb (runoff)                        = ',&
                                          tracer_input_icb,' J'
         !Pedro
     ...
     elseif (T_prog(n)%name(1:3) =='age') then
         ...
         write (stdoutunit,'(a,es24.17,a)') ' Age input via icb (runoff)                          = ',&
         ...
         write (stdoutunit,'(a,es24.17,a)') ' Tracer input via icb (runoff)                       = ',&
                                          tracer_input_icb,' kg'
         !Pedro
...
! </DESCRIPTION>
!
subroutine mass_conservation (Time, Thickness, Ext_mode, pme, runoff, calving, basal, icb)
  ...
  if (itt==itte_mass) then 
      ...
      mass_input       = pme_input + runoff_input + calving_input + basal_input + icb_input + source_input &
      ...
      !Pedro
      write (stdoutunit,'(a,es24.17,a)') &
      ' Mass input via basal (runoff) fluxes over time interval      = ',basal_input,' kg.'
      write (stdoutunit,'(a,es24.17,a)') &
      ' Mass input via icb (runoff) fluxes over time interval      = ',icb_input,' kg.'
      !Pedro
      write (stdoutunit,'(a,es24.17,a)') &
