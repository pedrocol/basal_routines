...
!Pedro
  ...
  real, dimension(isd:ied,jsd:jed)      :: icb         ! mass flux of river (runoff+calving) per horz area (kg/(s*m^2))
  real, dimension(isd:ied,jsd:jed,2)    :: uicb        ! horizontal velocity from river runoff+calving
  real, dimension(isd:ied,jsd:jed,nk)      :: icb3d         ! mass flux of river (runoff+calving) per horz area (kg/(s*m^2))
  real, dimension(isd:ied,jsd:jed,nk,2)    :: uicb3d        ! horizontal velocity from river runoff+calving
!Pedro
...
!Pedro
  ...
  real, pointer, dimension(:,:)     :: icb               =>NULL() ! mass flux of river runoff (liquid) per horz area from (kg/(s*m^2)) 
  real, pointer, dimension(:,:,:)   :: uicb              =>NULL() ! horizontal velocity from river (m/s)
  real, pointer, dimension(:,:,:)     :: icb3d               =>NULL() ! mass flux of river runoff (liquid) per horz area from (kg/(s*m^2)) 
  real, pointer, dimension(:,:,:,:)   :: uicb3d              =>NULL() ! horizontal velocity from river (m/s)
!Pedro
...
!Pedro
    ...
    allocate(uicb(isd:ied,jsd:jed,2))
    allocate(icb3d(isd:ied,jsd:jed,nk))
    allocate(uicb3d(isd:ied,jsd:jed,nk,2))
!Pedro
  ...
  subroutine update_ocean_model(Ice_ocean_boundary, Ocean_state, Ocean_sfc, &
    ...
    do num_ocn = 1,num_ocean_calls
       ...
       call flux_adjust(Time, T_diag(1:num_diag_tracers), Dens, Ext_mode, &
                        T_prog(1:num_prog_tracers), Velocity, river, melt, pme, basal, icb)
...
!Pedro       
       ...
       call basal_tracer_source(Time, Time_steps,Thickness, Dens, T_prog(1:num_prog_tracers), &
                                basal, diff_cbt, index_temp, index_salt, basal3d)
       call mpp_clock_end(id_basal_tracer)
       ! add icb to T_prog%th_tendency 
       call mpp_clock_begin(id_icb_tracer)
       call icb_tracer_source(Time, Time_steps,Thickness, Dens, T_prog(1:num_prog_tracers), &
                                icb, diff_cbt, index_temp, index_salt, icb3d)
...
!Pedro
       ...
       call eta_and_pbot_tendency(Time, pme, river, basal, icb, Ext_mode, use_blobs)
       !Pedro
       ...
       if (use_blobs) then
          ...
          call ocean_advection_velocity(Velocity, Time, Thickness, Dens, pme, river, basal, icb, &
                                        Adv_vel, Lagrangian_system, basal3d, icb3d, use_blobs)
          !Pedro
          ...
          call ocean_advection_velocity(Velocity, Time, Thickness, Dens, pme, river, basal, &
                                        icb, Adv_vel, Lagrangian_system, basal3d, icb3d, use_blobs)
          !Pedro
          call mpp_clock_end(id_advect)
       ...
       if(Ocean_options%vertmix==VERTMIX_GOTM) then 
          ...
          call advect_gotm_compute(Time, Adv_vel, Thickness, pme, river, basal, icb)
          !Pedro
    ...
    !Pedro 
    call energy_analysis (Time, Thickness, Ext_mode, Adv_vel, Dens, pme, river, basal, icb, &
                          upme, uriver, ubasal, uicb, visc_cbu, visc_cbt, visc_cbu_form_drag, Velocity, &
                          basal3d, ubasal3d, icb3d, uicb3d)
    !Pedro
    ...
    call ocean_diagnostics(Time, Thickness, T_prog(1:num_prog_tracers), T_diag(1:num_diag_tracers), &
                           Adv_vel, Ext_mode, Dens, Velocity, &
                           pme, melt, runoff, calving, visc_cbt, diff_cbt, basal, icb)
    !Pedro
