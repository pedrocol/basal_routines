...
subroutine ocean_icb_tracer_init(Grid, Domain, Time, T_prog, dtime, Ocean_options, Dens)
  ...
  do n=1,num_prog_tracers
     id_icb_tend(n) = register_diag_field ('ocean_model', trim(T_prog(n)%name)//'_icb_fw_tend',&
        ...
        if (fwfisf(i,j) > 0.0 .and. Grd%kmt(i,j) > 0) then
           ...
           n = index_temp
              ...
              if ( trim(T_prog(n)%name) == 'temp' ) then
                 ...
                 T_prog(n)%icb_tracer_flux(i,j)= &
                   Grd%tmask(i,j,1)*T_prog(index_temp)%ticb(i,j)*fwfisf(i,j)

              endif !Do nothing for salt and age

              do k=misfkt(i,j),misfkb(i,j)
                 !Dilution is performed via mass add
                 Thickness%mass_source(i,j,k) = Thickness%mass_source(i,j,k) +  icb3d(i,j,k)
              enddo
