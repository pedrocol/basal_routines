...
subroutine barotropic_diag_init(Time)
  ...
  id_icb_total = register_diag_field ('ocean_model', 'icb_total',       &
                   Time%model_time,'mass icb added over dtuv time', 'kg', &
                   missing_value=missing_value, range=(/-10.0,1e18/))
  !Pedro
...
! </DESCRIPTION>
!
subroutine eta_and_pbot_diagnose (Time, Dens, Thickness, patm, pme, river, basal, icb, &
  ...
  call barotropic_integrals (Time, Ext_mode, patm, pme, river, basal, icb)
  !Pedro
  ! diagnostics 
...
! </DESCRIPTION>
!
subroutine eta_and_pbot_tendency(Time, pme, river, basal, icb, Ext_mode, use_blobs)
         ...
         do i=isd,ied       
            Ext_mode%deta_dt(i,j) = Grd%tmask(i,j,1)                              &
                 *rho0r*( Ext_mode%conv_rho_ud_t(i,j,tau) + pme(i,j) + river(i,j) &
                 + basal(i,j) + icb(i,j) + Ext_mode%source(i,j) ) !Pedro
         enddo
      enddo
      if (use_blobs) then
         do j=jsd,jed
...
subroutine update_ocean_barotropic (Time, Dens, Thickness, Adv_vel, &
                                    Ext_mode, patm, pme, river, basal, icb, use_blobs)
  ...
  if(splitting) then 
      ...
      if(horz_grid == MOM_BGRID) then 
         if(vert_coordinate_class==DEPTH_BASED) then 
           ...
           call pred_corr_tropic_depth_bgrid (Time, Thickness, Ext_mode, patm, pme, river, basal, icb, use_blobs)
           !Pedro
         elseif(vert_coordinate_class==PRESSURE_BASED ) then 
           !Pedro
           !call pred_corr_tropic_press_bgrid (Time, Thickness, Ext_mode, pme, river, use_blobs)
           call pred_corr_tropic_press_bgrid (Time, Thickness, Ext_mode, pme, river, basal, icb, use_blobs)
           !Pedro
         endif  
      else 
         if(vert_coordinate_class==DEPTH_BASED) then 
           ...
           call pred_corr_tropic_depth_cgrid (Time, Thickness, Ext_mode, patm, pme, river, basal, icb, use_blobs)
           !Pedro
         elseif(vert_coordinate_class==PRESSURE_BASED ) then 
           !Pedro
           !call pred_corr_tropic_press_cgrid (Time, Thickness, Ext_mode, pme, river, use_blobs)
           call pred_corr_tropic_press_cgrid (Time, Thickness, Ext_mode, pme, river, basal, icb, use_blobs)
           !Pedro
         endif  
      endif 
  ...
  call eta_terms_diagnose(Time, Dens, Thickness, Ext_mode, pme, river, basal, icb)
  !Pedro
...
! </DESCRIPTION>
!
subroutine pred_corr_tropic_depth_bgrid (Time, Thickness, Ext_mode, patm, pme, river, basal, icb, use_blobs)
...
!        eta_t_bt(i,j,fstaup1) = Ext_mode%eta_t_bar(i,j,tau)
        ...
        steady_forcing(i,j) = Grd%tmask(i,j,1)*( pme(i,j) + river(i,j) + basal(i,j) + icb(i,j) + &
...
! </DESCRIPTION>
!
subroutine pred_corr_tropic_depth_cgrid (Time, Thickness, Ext_mode, patm, pme, river, basal, icb, use_blobs)
...
! </DESCRIPTION>
!
subroutine pred_corr_tropic_press_bgrid (Time, Thickness, Ext_mode, pme, river, basal, icb, use_blobs)
  ...
  do j=jsd,jed
     do i=isd,ied
        ...
        steady_forcing(i,j) = Grd%tmask(i,j,1)                                            &
        *(grav*(pme(i,j) + river(i,j) + basal(i,j) + icb(i,j) + Ext_mode%source(i,j) &
...
! </DESCRIPTION>
!
subroutine pred_corr_tropic_press_cgrid (Time, Thickness, Ext_mode, pme, river, basal, icb, use_blobs)
...
! </DESCRIPTION>
!
subroutine barotropic_integrals (Time, Ext_mode, patm, pme, river, basal, icb)
  ...
  if (diag_step > 0 .and. mod(Time%itt, diag_step) == 0&
       ...
       .or. need_data(id_basal_total,next_time) .or. need_data(id_icb_total,next_time)) then  
      ...
      if(have_obc) then
        do j=jsc,jec
          do i=isc,iec
            ...
            icb_total = icb_total + Grd%dat(i,j)*Grd%tmask(i,j,1)*icb(i,j)*Grd%obc_tmask(i,j)
            !Pedro
        ...
        do j=jsc,jec
          do i=isc,iec
            ...
            icb_total = icb_total + Grd%dat(i,j)*Grd%tmask(i,j,1)*icb(i,j)
            !Pedro
            mass_total  = mass_total  + Grd%dat(i,j)*Grd%tmask(i,j,1) &
      ...
      if (id_icb_total > 0) used = send_data (id_icb_total, icb_total, Time%model_time)
      !Pedro
...
! </DESCRIPTION>
!
subroutine eta_terms_diagnose(Time, Dens, Thickness, Ext_mode, pme, river, basal, icb)
