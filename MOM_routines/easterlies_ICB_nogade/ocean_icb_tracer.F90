...
!
!<OVERVIEW>
! Add icb fresh water flux to ocean.
!</OVERVIEW>
!
!<DESCRIPTION>
! This module applies a 3D fresh water flux, presumably from icb 
! melting (but the source of the FW is arbitrary). The code is based
! on ocean_rivermix.F90. The icb meltwater input file is from 
...
!</DESCRIPTION>
!
!<NAMELIST NAME="ocean_icb_tracer_nml">
...
private watermass_diag_icb

#include <ocean_memory.h>

type ocean_icb_type
   integer :: id                                             ! time_interp_external index
   character(len=32) :: name                                 ! tracer name corresponding to icb
...
public ocean_icb_tracer_init
public icb_tracer_source

character(len=126)  :: version = '$Id: ocean_icb_tracer.F90,v 20.0 2013/12/14 00:16:24 fms Exp $'
character (len=128) :: tagname = '$Name: tikal $'

! for diagnostics 
logical :: used
integer, dimension(:), allocatable :: id_icb_tend
integer :: id_icb_fwflx        =-1
integer :: id_icb_fwflx2d        =-1
...
integer :: id_neut_rho_icbmix_on_nrho   =-1
integer :: id_wdian_rho_icbmix_on_nrho   =-1
integer :: id_tform_rho_icbmix_on_nrho   =-1
...
integer :: id_eta_tend_icbmix        =-1
integer :: id_eta_tend_icbmix_glob   =-1
...
logical :: use_icb_after_init = .false.
...
namelist /ocean_basal_tracer_nml/ use_basal_module, use_icb_module, test_nml, damp_coeff_3d

contains


!#######################################################################
! <SUBROUTINE NAME="ocean_icb_tracer_init">
!
! <DESCRIPTION>
! This subroutine is intended to be used to initialize addition of icb melt
! water fluxes from an input file. 
! Everything in this subroutine is a user prototype, and should be replacable.
! </DESCRIPTION>
!
subroutine ocean_icb_tracer_init(Grid, Domain, Time, T_prog, dtime, Ocean_options, Dens)
  ...
  if ( module_is_initialized ) then 
    ...
    '==>Error in ocean_icb_tracer_mod (ocean_icb_tracer_init): module already initialized')
  endif 
  ...
  if(use_icb_module) then
      write(stdoutunit,*)'==>Note from ocean_icb_tracer_mod: Using this module.'
      Ocean_options%ocean_icb_tracer= 'Used ocean tracer icb.'
  else
      write(stdoutunit,*)'==>Note from ocean_icb_tracer_mod: NOT using ocean tracer icb.'
      Ocean_options%ocean_icb_tracer= 'Did NOT use ocean tracer icb.'
      return
  endif
  ...
  id_icb_fwflx = register_diag_field ('ocean_model','icb_fwflx', Grd%tracer_axes(1:3),   &
              ...
              standard_name='water_flux_into_sea_water_from_icb_melting')
  id_icb_fwflx2d = register_diag_field ('ocean_model','icb_fwflx2d', Grd%tracer_axes(1:2),   &
              Time%model_time, '2d mass flux of liquid icb meltwater entering ocean ',        &
              '(kg/m^3)*(m/sec)', missing_value=missing_value,range=(/-1e6,1e6/),            &
              standard_name='water_flux_into_sea_water_from_icb_melting')

  
  allocate (id_icb_tend(num_prog_tracers))
  id_icb_tend = -1

  do n=1,num_prog_tracers
     id_icb_tend(n) = register_diag_field ('ocean_model', 'icb_fw_tend',&
               Grd%tracer_axes(1:3), Time%model_time, 'rho*dzt*tendency due to icb freshwater flux',          &
               trim(T_prog(n)%flux_units), missing_value=missing_value, range=(/-1.e10,1.e10/))
  enddo

  ! register for diag_manager
  num_prog_tracers = size(T_prog(:))
  allocate (id_icbfw(num_prog_tracers))
  allocate (id_icbmix_on_nrho(num_prog_tracers))
  id_icbfw   = -1
  id_icbmix_on_nrho   = -1
  do n=1,num_prog_tracers
     if(T_prog(n)%name == 'temp') then
        id_icbfw(n) = register_diag_field ('ocean_model', trim(T_prog(n)%name)//'_icbmix', &
                         Grd%tracer_axes(1:3), Time%model_time,                                 &
                         'cp*icbmix*rho_dzt*temp', 'Watt/m^2',                                &
                         missing_value=missing_value, range=(/-1.e10,1.e10/))
        id_icbmix_on_nrho(n) = register_diag_field ('ocean_model', trim(T_prog(n)%name)//'_icbmix_on_nrho', &
                         Dens%neutralrho_axes(1:3), Time%model_time,                                 &
                         'cp*icbmix*rho_dzt*temp binned to neutral density', 'Watt/m^2',       &
                         missing_value=missing_value, range=(/-1.e20,1.e20/))
     else
        id_icbfw(n) = register_diag_field ('ocean_model', trim(T_prog(n)%name)//'_icbmix', &
                         Grd%tracer_axes(1:3), Time%model_time,                                 &
                         'icbmix*rho_dzt*tracer for '//trim(T_prog(n)%name),                  &
                         trim(T_prog(n)%flux_units),                                            &
                         missing_value=missing_value, range=(/-1.e10,1.e10/))
        id_icbmix_on_nrho(n) = register_diag_field ('ocean_model', trim(T_prog(n)%name)//'_icbmix_on_nrho', &
                         Dens%neutralrho_axes(1:3), Time%model_time,                                 &
                         'icbmix*rho_dzt*tracer for '//trim(T_prog(n)%name)//' binned to neutral density',&
                         trim(T_prog(n)%flux_units),                                            &
                         missing_value=missing_value, range=(/-1.e10,1.e10/))
    endif
  enddo
...
end subroutine ocean_icb_tracer_init
! </SUBROUTINE> NAME="ocean_icb_tracer_init"


!#######################################################################
! <SUBROUTINE NAME="icb_tracer_source">
!
! <DESCRIPTION>
...
!#######################################################################
           ...
           do n=1,num_prog_tracers
              ...
              if (gade_line == 0 ) then
                 ...
                 if ( trim(T_prog(nn)%name) /= 'temp' ) then !salt and age
                    ...
                    do k=misfkt(i,j),misfkb(i,j)
                       icb3d(i,j,k) = fwfisf(i,j)*delta(k)
                       zinsert = fwfisf(i,j)*dtime*delta(k)
                       tracernew(k) = (T_prog(nn)%field(i,j,k,tau)*Thickness%rho_dzt(i,j,k,tau) + ticb*zinsert) / &
              ...
              elseif (gade_line == 1 ) then
                 ...
                 if ( trim(T_prog(n)%name) == 'temp' ) then
                    ...
                    ticb =  max(0.0,T_prog(index_temp)%field(i,j,1,tau))
                 endif
                 ...
                 if ( method == 0 ) then
                    ...
                    do k=misfkb(i,j),misfkt(i,j),-1
                       ...
                       tracernew(k) = (tracerextra*zextra + T_prog(n)%field(i,j,k,tau)*Thickness%rho_dzt(i,j,k,tau) + &
                                      ticb*zinsert) / (zextra+Thickness%rho_dzt(i,j,k,tau)+zinsert)
                       icb3d(i,j,k) = ( zextra + zinsert ) / ( dtime*delta(k) ) !Equivalente icb flux
                 ...
                 elseif ( method == 1 ) then
                    do k=misfkt(i,j),misfkb(i,j)
                       icb3d(i,j,k) = fwfisf(i,j)*delta(k)
                       zinsert = fwfisf(i,j)*dtime*delta(k)
                       tracernew(k) = (T_prog(n)%field(i,j,k,tau)*Thickness%rho_dzt(i,j,k,tau) + ticb*zinsert) / &
                                      (Thickness%rho_dzt(i,j,k,tau)+zinsert)
                       ticb_sum = ticb_sum + ticb*delta(k)
                    enddo
                    firstlev = misfkt(i,j)                 
  ...
  !!Diags
  if (id_ticb > 0) then !icb temp
     wrk1_2d=0.0
     do j=jsc,jec
        do i=isc,iec
           if ( fwfisf(i,j) /= 0.0 ) then
              wrk1_2d(i,j) = T_prog(index_temp)%ticb(i,j)
           endif
        enddo
     enddo
     call diagnose_2d(Time, Grd, id_ticb, wrk1_2d(:,:))
  endif

  if (id_icb_fwflx > 0) then !icb flux
     ! icb entering the ocean (kg/m^3)*(m/s)
     call diagnose_3d(Time, Grd, id_icb_fwflx, icb3d(:,:,:))
  endif

  if (id_icb_fwflx2d> 0) then !icb flux
     ! icb entering the ocean (kg/m^3)*(m/s)
     call diagnose_2d(Time, Grd, id_icb_fwflx2d, fwfisf(:,:))
  endif


  do n=1,num_prog_tracers
     if(id_icbfw(n) > 0) then !temp/salt flux
        call diagnose_3d(Time, Grd, id_icbfw(n), T_prog(n)%wrk1(:,:,:)*T_prog(n)%conversion)
     endif
     if(id_icbmix_on_nrho(n) > 0) then
        call diagnose_3d_rho(Time, Dens, id_icbmix_on_nrho(n), T_prog(n)%wrk1*T_prog(n)%conversion)
     endif
  enddo
...
subroutine watermass_diag_init_ba(Time, Dens)
  ...
  id_neut_rho_icbmix = register_diag_field ('ocean_model', 'neut_rho_icbmix',&
     Grd%tracer_axes(1:3), Time%model_time,                                      &
     'update of locally ref potrho from icbmix scheme',                        &
     '(kg/m^3)/sec', missing_value=missing_value, range=(/-1e20,1e20/))
  if(id_neut_rho_icbmix > 0) compute_watermass_diag_ba = .true.

  id_neut_rho_icbmix_on_nrho = register_diag_field ('ocean_model',                    &
    'neut_rho_icbmix_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,            &
    'update of locally ref potrho from icbmix scheme as binned to neutral rho layers',&
    '(kg/m^3)/sec', missing_value=missing_value, range=(/-1e20,1e20/))
  if(id_neut_rho_icbmix_on_nrho > 0) compute_watermass_diag_ba = .true.

  id_wdian_rho_icbmix = register_diag_field ('ocean_model', 'wdian_rho_icbmix',&
     Grd%tracer_axes(1:3), Time%model_time,                                        &
     'dianeutral mass transport due to icbmix scheme',                           &
     'kg/sec', missing_value=missing_value, range=(/-1e20,1e20/))
  if(id_wdian_rho_icbmix > 0) compute_watermass_diag_ba = .true.

  id_wdian_rho_icbmix_on_nrho = register_diag_field ('ocean_model',                  &
    'wdian_rho_icbmix_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,          &
    'dianeutral mass transport due to icbmix scheme as binned to neutral rho layers',&
    'kg/sec', missing_value=missing_value, range=(/-1e20,1e20/))
  if(id_wdian_rho_icbmix_on_nrho > 0) compute_watermass_diag_ba = .true.

  id_tform_rho_icbmix = register_diag_field ('ocean_model', 'tform_rho_icbmix',&
     Grd%tracer_axes(1:3), Time%model_time,                                        &
     'transform due to icbmix scheme pre-binning to neutral rho',                &
     'kg/sec', missing_value=missing_value, range=(/-1e20,1e20/))
  if(id_tform_rho_icbmix > 0) compute_watermass_diag_ba = .true.

  id_tform_rho_icbmix_on_nrho = register_diag_field ('ocean_model',          &
    'tform_rho_icbmix_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,  &
    'watermass transform from icbmix scheme as binned to neutral rho layers',&
    'kg/sec', missing_value=missing_value, range=(/-1e20,1e20/))
  if(id_tform_rho_icbmix_on_nrho > 0) compute_watermass_diag_ba = .true.
  ...
  id_neut_rho_pbl_ba_pr = register_diag_field ('ocean_model',   &
   ...
   'process advective-form material time derivative from icb',&
  ...
  id_neut_rho_pbl_ba_pr_on_nrho = register_diag_field ('ocean_model',                        &
   ...
   'process advective-form material time derivative from icb as binned to neutral density',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_rho_pbl_ba_pr = register_diag_field ('ocean_model',  &
   ...
   'process advective-form dianeutral transport from icb',    &
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_rho_pbl_ba_pr_on_nrho = register_diag_field ('ocean_model',                    &
    ...
    'process advective-form dianeutral transport from icb as binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_rho_pbl_ba_pr = register_diag_field ('ocean_model',                              &
    ...
    'process advective-form water mass transform from icb pre-binning to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  if(id_tform_rho_pbl_ba_pr > 0) compute_watermass_diag_ba=.true.
   ...
   'process advective-form water mass transform from icb binned to neutral density',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_temp_pbl_ba_pr = register_diag_field ('ocean_model',               &
   ...
   'temp related process advective-form material time derivative from icb',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_temp_pbl_ba_pr_on_nrho = register_diag_field ('ocean_model',                                 &
   ...
   'temp related process advective-form material time derivative from icb binned to neutral density',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_temp_pbl_ba_pr = register_diag_field ('ocean_model',          &
   ...
   'temp related process advective-form dianeutral transport from icb',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_temp_pbl_ba_pr_on_nrho = register_diag_field ('ocean_model',                             &
    ...
    'temp related process advective-form dianeutral transport from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_temp_pbl_ba_pr = register_diag_field ('ocean_model',           &
    ...
    'temp related process advective-form water mass transform from icb',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_temp_pbl_ba_pr_on_nrho = register_diag_field ('ocean_model',                            &
   ...
   'temp related process advective-form water mass transform from icb binned to neutral density',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_salt_pbl_ba_pr_on_nrho = register_diag_field ('ocean_model',                                 &
   ...
   'salt related process advective-form material time derivative from icb binned to neutral density',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_salt_pbl_ba_pr = register_diag_field ('ocean_model',          &
   ...
   'salt related process advective-form dianeutral transport from icb',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_salt_pbl_ba_pr_on_nrho = register_diag_field ('ocean_model',                             &
    ...
    'salt related process advective-form dianeutral transport from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_salt_pbl_ba_pr = register_diag_field ('ocean_model',           &
    ...
    'salt related process advective-form water mass transform from icb',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_salt_pbl_ba_pr_on_nrho = register_diag_field ('ocean_model',                            &
   ...
   'salt related process advective-form water mass transform from icb binned to neutral density',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_rho_pbl_ba_kn = register_diag_field ('ocean_model',     &
   ...
   'kinematic advective-form material time derivative from icb',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_rho_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                           &
    ...
    'kinematic advective-form material time derivative from icb as binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_rho_pbl_ba_kn = register_diag_field ('ocean_model',   &
    ...
    'kinematic advective-form dianeutral transport from icb',  &
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_rho_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                      &
    ...
    'kinematic advective-form dianeutral transport from icb as binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_rho_pbl_ba_kn = register_diag_field ('ocean_model',                                &
    ...
    'kinematic advective-form water mass transform from icb pre-binning to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_rho_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                   &
    ...
    'kinematic advective-form water mass transform from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_temp_pbl_ba_kn = register_diag_field ('ocean_model',                 &
   ...
   'temp related kinematic advective-form material time derivative from icb',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_temp_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                                    &
    ...
    'temp related kinematic advective-form material time derivative from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_temp_pbl_ba_kn = register_diag_field ('ocean_model',             &
    ...
    'temp related kinematic advective-form dianeutral transport from icb',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_temp_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                               &
    ...
    'temp related kinematic advective-form dianeutral transport from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_temp_pbl_ba_kn = register_diag_field ('ocean_model',             &
    ...
    'temp related kinematic advective-form water mass transform from icb',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_temp_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                               &
    ...
    'temp related kinematic advective-form water mass transform from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_salt_pbl_ba_kn = register_diag_field ('ocean_model',                 &
   ...
   'salt related kinematic advective-form material time derivative from icb',&
   'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_neut_salt_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                                    &
    ...
    'salt related kinematic advective-form material time derivative from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_salt_pbl_ba_kn = register_diag_field ('ocean_model',             &
    ...
    'salt related kinematic advective-form dianeutral transport from icb',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_wdian_salt_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                               &
    ...
    'salt related kinematic advective-form dianeutral transport from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_salt_pbl_ba_kn = register_diag_field ('ocean_model',             &
    ...
    'salt related kinematic advective-form water mass transform from icb',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  ...
  id_tform_salt_pbl_ba_kn_on_nrho = register_diag_field ('ocean_model',                               &
    ...
    'salt related kinematic advective-form water mass transform from icb binned to neutral density',&
    'kg/sec', missing_value=missing_value, range=(/-1.e20,1.e20/))
  if(id_tform_salt_pbl_ba_kn_on_nrho > 0) compute_watermass_diag_ba=.true.

  ! eta tendency terms 
  id_eta_tend_icbmix= -1
  id_eta_tend_icbmix= register_diag_field ('ocean_model','eta_tend_icbmix', &
       Grd%tracer_axes(1:2), Time%model_time,                                   &
       'non-Bouss steric sea level tendency from icbmix', 'm/s',              &
       missing_value=missing_value, range=(/-1e10,1.e10/))
  if(id_eta_tend_icbmix > 0) compute_watermass_diag_ba = .true.

  id_eta_tend_icbmix_glob= -1
  id_eta_tend_icbmix_glob= register_diag_field ('ocean_model', 'eta_tend_icbmix_glob',&
       Time%model_time,                                                                   &
       'global mean non-bouss steric sea level tendency from icbmix',                   &
       'm/s', missing_value=missing_value, range=(/-1e10,1.e10/))
  if(id_eta_tend_icbmix_glob > 0) compute_watermass_diag_ba = .true.

    if(compute_watermass_diag_ba) then
    write(stdoutunit,'(/a/)') &
    '==>Note: running ocean_icbmix_mod w/ compute_watermass_diag_ba=.true.'
  endif
...
! </DESCRIPTION>
!
subroutine watermass_diag_icb(Time, Dens, T_prog, icb, &
  ...
  call diagnose_3d(Time, Grd, id_neut_rho_icbmix, wrk2(:,:,:))
  call diagnose_3d(Time, Grd, id_wdian_rho_icbmix, wrk3(:,:,:))
  call diagnose_3d(Time, Grd, id_tform_rho_icbmix, wrk4(:,:,:))
  call diagnose_3d_rho(Time, Dens, id_neut_rho_icbmix_on_nrho, wrk2)
  call diagnose_3d_rho(Time, Dens, id_wdian_rho_icbmix_on_nrho, wrk3)
  call diagnose_3d_rho(Time, Dens, id_tform_rho_icbmix_on_nrho, wrk4)

  if(id_eta_tend_icbmix > 0 .or. id_eta_tend_icbmix_glob > 0) then
      eta_tend(:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
         ...
         enddo
      enddo
      call diagnose_2d(Time, Grd, id_eta_tend_icbmix, eta_tend(:,:))
      call diagnose_sum(Time, Grd, Dom, id_eta_tend_icbmix_glob, eta_tend, cellarea_r)
  endif
  ...
  do j=jsc,jec
     do i=isc,iec
        do k=misfkt(i,j),misfkb(i,j)
           wrk1(i,j,k) = Grd%tmask(i,j,k)*icb(i,j)                            &
...
end module ocean_icb_tracer_mod
