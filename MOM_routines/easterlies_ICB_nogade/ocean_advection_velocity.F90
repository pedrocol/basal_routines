...
subroutine ocean_advection_velocity (Velocity, Time, Thickness, Dens, pme, river, basal, &
                                     icb, Adv_vel, L_system, basal3d, icb3d, use_blobs)
  ...
  real, dimension(isd:ied,jsd:jed,nk)   :: basal3d_u
  real, dimension(isd:,jsd:),     intent(in)    :: icb
  real, dimension(isd:,jsd:,:),     intent(in)    :: icb3d
  real, dimension(isd:ied,jsd:jed,nk)   :: icb3d_u
  !Pedro
  type(ocean_adv_vel_type),   intent(inout) :: Adv_vel
  ...
  if(.not. prescribe_advection_velocity .and. horz_grid == MOM_BGRID) then 
      ...
      Adv_vel%wrho_bt(:,:,0) = -(pme(:,:) + river(:,:) + icb(:,:)) !+ basal(:,:)) !Pedro
      ...
      if (use_blobs) then
         ...
         do k=1,nk
            tmp(:,:) = Thickness%rho_dzt_tendency(:,:,k) - Thickness%mass_source(:,:,k) - basal3d(:,:,k) - icb3d(:,:,k)!Pedro
            Adv_vel%wrho_bt(:,:,k) = (tmp(:,:) + Adv_vel%diverge_t(:,:,k) + Adv_vel%wrho_bt(:,:,k-1)) &
                                     *Grd%tmask(:,:,k)
            tmp(:,:) = Grd%umask(:,:,k)*REMAP_BT_TO_BU(tmp(:,:))
            Adv_vel%wrho_bu(:,:,k) = tmp(:,:) + Adv_vel%diverge_u(:,:,k) + Adv_vel%wrho_bu(:,:,k-1) 
